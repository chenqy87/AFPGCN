from Graph_Client_TimelyFL import Graph_Client_TimelyFL
import os
import torch
import numpy as np
...
class TimelyFL_Graph(nn.Module):
  def __init__(self, device_num,number):
    super(TimelyFL_Graph, self).__init__()
    ...
    self.writer = SummaryWriter('./results/s_{}_b_{}_c_{}_k_{}_i_{}_n_{}/TimelyFL_Graph'.format(TEST_UPDATE_STEP,BATCH_SIZE,IN_CHANNELS,self.K,BATCH_INDEX,self.number))
    self.clients = []
    ...
    for city in folder_city_set:
      self.clients.append(Graph_Client_TimelyFL(city,device_num,"train"))
  ...
  def Fed_Train(self,round):
    ...
    for id,j in enumerate(id_train_0):
      ...
      self.clients[j].Client_Train(self.min_time)
    ...
    self.min_time = np.sort(np.array(time_1))[self.num_clients - 1]

    for id in id_train_0:
      self.clients[id].time = max(self.clients[id].time - self.min_time,0)
    ...
    time_end = time.time()
    time_total = time_end - time_start + self.min_time
