...
class Graph_Client_TimelyFL(nn.Module):
    def __init__(self,city,device_num,mode):
        super(Graph_Client_TimelyFL, self).__init__()
    ...
    def Client_Train(self,time_k):
        ...
        e = max(1,int((time_k - self.t_com) / self.t_cmp) + 1)
        alpha = min(1,time_k / (self.t_cmp + self.t_com))
        alpha = max(0.1,alpha)
        layer_threshold = int(alpha * self.num_layers)
        for index,param in enumerate(self.net.parameters()):
            if index <= layer_threshold:
    ...
    def Client_Test(self,):
        ...
        #         self.optimizer.step()
                # d_lr.step()
        test_result = predict_and_save_results_mstgcn(self.net, self.test_loader, self.test_target_tensor, self.metric_method,self._mean, self._std, self.train_target_mean, self.train_target_std, self.actual_node_number)
