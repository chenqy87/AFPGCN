from Graph_Client_FedMeta import Graph_Client_FedMeta
import os
import torch
import numpy as np
...
class FedMeta_Graph(nn.Module):
  def __init__(self, device_num,number):
    super(FedMeta_Graph, self).__init__()
    ...
    # self.train_clients = []
    # self.test_clients = []
    # self.train_folder = r"./data/train_data"
    # self.test_folder = r'./data/test_data'
    # train_folder_city_set = os.listdir(self.train_folder)
    # test_folder_city_set = os.listdir(self.test_folder)
    # train_folder_city_set = [i.split("_")[0] for i in train_folder_city_set]
    # test_folder_city_set = [i.split("_")[0] for i in test_folder_city_set]

    # for city in train_folder_city_set:
    #   self.train_clients.append(Graph_Client_FedMeta(city,device_num,"train"))
    # for city in test_folder_city_set:
    #   self.test_clients.append(Graph_Client_FedMeta(city,device_num,"test"))
    self.writer = SummaryWriter('./results/s_{}_b_{}_c_{}_k_{}_i_{}_n_{}/FedMeta_Graph'.format(TEST_UPDATE_STEP,BATCH_SIZE,IN_CHANNELS,self.K,BATCH_INDEX,self.number))
    self.clients = []
    ...
    for city in folder_city_set:
      self.clients.append(Graph_Client_FedMeta(city,device_num,"train"))
