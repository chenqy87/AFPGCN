...
class ARIMA_Client(nn.Module):
    def __init__(self,city,device_num,mode):
        super(ARIMA_Client, self).__init__()
    ...
    def Client_Test(self,):
        ...
        for batch_index, batch_data in enumerate(self.test_loader):
            encoder_inputs, labels = batch_data
            input_0 = encoder_inputs[:3,1,0,:].cpu().numpy()
            output_0 = labels[:3,1,0].cpu().numpy()
            input_all = deepcopy(input_0)
            output_all = deepcopy(output_0)
            results = []
            for i in range(input_all.shape[0]):
                self.net = ARIMA(input_all[i],order = (12,0,12)).fit()
                results.append(self.net.forecast()[-1])
            break
