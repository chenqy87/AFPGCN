from Graph_Client_FedBuff import Graph_Client_FedBuff
import os
...
from config_reader import IN_CHANNELS,NB_BLOCK,K,NB_CHEV_FILTER,NB_TIME_FILTER,TIME_STRIDES,NUM_FOR_PREDICT,LEN_INPUT,NODES,TEST_UPDATE_STEP,BATCH_SIZE,BATCH_INDEX,\
  NUM_OF_DAYS,NUM_OF_HOURS,NUM_OF_WEEKS,LEARNING_RATE

class FedBuff_Graph(nn.Module):
  def __init__(self, device_num,number):
    super(FedBuff_Graph, self).__init__()
    ...
    self.writer = SummaryWriter('./results/s_{}_b_{}_c_{}_k_{}_i_{}_n_{}/FedBuff_Graph'.format(TEST_UPDATE_STEP,BATCH_SIZE,IN_CHANNELS,self.K,BATCH_INDEX,self.number))
    self.clients = []
    ...
    for city in folder_city_set:
      self.clients.append(Graph_Client_FedBuff(city,device_num,"train"))
  ...
  def Fed_Train(self,round):
    ...
    min_time = np.sort(np.array(time_1))[self.num_clients - 1]
    ...
    for id,j in enumerate(id_train):
      weight.append(1.0 / self.num_clients)
    ...
    for id,j in enumerate(id_train):
      ...
      for w,w_t in zip(self.net.parameters(),self.clients[j].net.parameters()):
        if (w.grad is None or id == 0):
          w_tem = Variable(torch.zeros_like(w)).to(self.device)
          w.grad = deepcopy(w_tem)
          # w.grad.data.copy_(w_tem.data)
        if w_t is None:
          w_t = Variable(torch.zeros_like(w)).to(self.device)
        w.grad.data.add_(w_t.grad.data*weight[id])
    self.optimizer.step()
    # self.
    time_end = time.time()
