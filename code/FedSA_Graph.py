from Graph_Client_FedSA import Graph_Client_FedSA
import os
import torch
import numpy as np
...
class FedSA_Graph(nn.Module):
  def __init__(self, device_num,number):
    super(FedSA_Graph, self).__init__()
    ...
    self.writer = SummaryWriter('./results/s_{}_b_{}_c_{}_k_{}_i_{}_n_{}/FedSA_Graph'.format(TEST_UPDATE_STEP,BATCH_SIZE,IN_CHANNELS,self.K,BATCH_INDEX,self.number))
    self.clients = []
    ...
    for city in folder_city_set:
      self.clients.append(Graph_Client_FedSA(city,device_num,"train"))
