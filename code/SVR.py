from SVR_Client import SVR_Client
import os
import torch
import numpy as np
...
class SVR(nn.Module):
  def __init__(self, device_num,number):
    ...
    self.writer = SummaryWriter('./results/s_{}_b_{}_c_{}_k_{}_i_{}_n_{}/SVR'.format(TEST_UPDATE_STEP,BATCH_SIZE,IN_CHANNELS,self.K,BATCH_INDEX,self.number))
    self.clients = []
    ...
    for city in folder_city_set:
      self.clients.append(SVR_Client(city,device_num,"train"))
  ...
  def Fed_Test(self,round,num):
    ...
    print("Round {}:\n".format(round))
    print('all MAE: %.5f' % (mae_mean))
    print('all RMSE: %.5f' % (rmse_mean))
    print('all MAPE: %.5f' % (mape_mean))
    print('all R2: %.2f' % (r2_mean*100))
