from Graph_Client_TWAFL import Graph_Client_TWAFL
import os
import torch
import numpy as np
...
class TWAFL_Graph(nn.Module):
  def __init__(self, device_num,number):
    super(TWAFL_Graph, self).__init__()
    ...
    self.writer = SummaryWriter('./results/s_{}_b_{}_c_{}_k_{}_i_{}_n_{}/TWAFL_Graph'.format(TEST_UPDATE_STEP,BATCH_SIZE,IN_CHANNELS,self.K,BATCH_INDEX,self.number))
    self.clients = []
    ...
    for city in folder_city_set:
      self.clients.append(Graph_Client_TWAFL(city,device_num,"train"))
  ...
  def Fed_Train(self,round):
    ...
    for id,j in enumerate(id_train_0):
      if self.clients[j].time <= 0:
        ...
        self.clients[j].Client_Train(self.refresh_layers)
    ...
    with torch.no_grad():
      ...
      for id,j in enumerate(id_train):
        ...
        for w,w_t in zip(self.net.parameters(),self.clients[j].net.parameters()):
          ...
          a += 1
          if layer_refresh_status < 0 & round%self.deep_frequency != 0:
            continue
